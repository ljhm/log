...
glog_cleanup() {
    ...
    local minimal_count_keep="3"
    if (( max_count_keep < minimal_count_keep )); then
        max_count_keep="$minimal_count_keep"
    fi

    local log_files=($(ls -1t "${log_dir}"/*.log."${log_level}".*))

    if (( ${#log_files[@]} > $max_count_keep )); then
        local num_to_delete=$((${#log_files[@]} - $max_count_keep))

        # Delete the oldest log files!!!
        for (( i=0; i<num_to_delete; i++ )); do
            local file="${log_files[$((max_count_keep + i))]}"
            echo "delete $file"
            rm "$file"
        done
    fi
...
main(){
    local LOG_DIR="./log" # path to the log files
    local MAX_COUNT_KEEP="3"
